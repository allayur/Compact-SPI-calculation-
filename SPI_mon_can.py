...
#Read hindcasts year by year and cut by rectangular
	for iy in range(yst, yfin+1):
		...
		dlat=np.abs(latitude_s[:]-llclat)
		dlon=np.abs(longitude_s[:]-llclon)
		all=np.where(dlat==np.min(dlat))
		bll=np.where(dlon==np.min(dlon))
		dlat2=np.abs(latitude_s[:]-urclat)
		dlon2=np.abs(longitude_s[:]-urclon)
		aur=np.where(dlat2==np.min(dlat2))
		bur=np.where(dlon2==np.min(dlon2))
		precip_s=precip_s0[:, all[0][0]:aur[0][0]+1, bll[0][0]:bur[0][0]+1]
		
		precip_s=np.reshape(precip_s, (1, precip_s0.shape[0], long))
...
# SPI simulation for historical forecasts (hindcasts) from Canadian Prediction Center
	...
	SPI_s=np.zeros((precip_s_all.shape[0], long))
	
	for ig in range(0, long):
		precip_s_amean=np.mean(precip_s_all[:,:,ig], axis=1)
		prs=precip_s_amean[:]
		qs=len(prs[prs<=0.])/len(prs)
		precip_snonz=prs[prs>0.]
		fit_alpha_s[ig], fit_loc_s[ig], fit_beta_s[ig] = stats.gamma.fit(precip_snonz, floc=0 ) 
		if (qs<thresh):
			for it in range(0,precip_s_all.shape[0]):
				G=gamma.cdf(precip_s_amean[it], fit_alpha_s[ig], scale=fit_beta_s[ig])
				H=qs+(1-qs)*G
				if (H<0.5):
					t=np.sqrt(np.log(1./(H*H)))
					SPI_s[it,ig]=-(t-((c0+c1*t+c2*t*t)/(1+d1*t+d2*t*t+d3*t*t*t)))
				else:						
					t=np.sqrt(np.log(1./((1.-H)*(1-H))))
					SPI_s[it, ig]=t-((c0+c1*t+c2*t*t)/(1+d1*t+d2*t*t+d3*t*t*t))
		else:
			SPI_s[it,ig]=noval
	SPI_out_hindc=np.reshape(SPI_s, (SPI_s.shape[0], a1, a2))			